...
func LogColumns() []*ckdb.Column {
	columns := []*ckdb.Column{
		...
		ckdb.NewColumn("span_id", ckdb.String).SetCodec(ckdb.CodecZSTD).SetIndex(ckdb.IndexBloomfilter).SetComment("Trace ID"),
		ckdb.NewColumn("trace_flags", ckdb.UInt32).SetComment("W3C trace flag, currently not support yet"),
		ckdb.NewColumn("severity_number", ckdb.UInt8).SetComment("numerical value of the severity(also known as log level id)"),
		ckdb.NewColumn("body", ckdb.String).SetCodec(ckdb.CodecZSTD).SetComment("log content"),
		ckdb.NewColumn("app_instance", ckdb.LowCardinalityString).SetComment("Application Instance (service name)"),
		...
		ckdb.NewColumn("attribute_values", ckdb.ArrayString).SetComment("the value of the extra attributes"),
...
func GenLogCKTable(cluster, storagePolicy, table string, ttl int, coldStorage *ckdb.ColdStorage) *ckdb.Table {
	...
	orderKeys := []string{"app_instance", timeKey, "l3_device_id"}
